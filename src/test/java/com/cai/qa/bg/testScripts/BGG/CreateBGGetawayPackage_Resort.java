...
public class CreateBGGetawayPackage_Resort extends BaseTest { 
	
	ExcelDataProvider excel = new ExcelDataProvider();
	public static Actions actions;
	
	private Logger log = LoggerHelp.getLogger(CreateBGGetawayPackage_Resort.class);
	...
	public void SelectDestination(){
		...
		test.log(Status.INFO,"LasVegas Destination is selected");
		
		actions = new Actions(driver);
		actions.moveToElement(bggPage.destinations).perform();
		threadsleepdelay(3000);
		bggPage.LasVegas.click();
		threadsleepdelay(3000);
		
		//************ VERIFICATION ****************
		Assert.assertEquals(bggPage.destinationSearchResultsLasVegas.isDisplayed(), true);
	}
